                ...
                withCredentials([string(credentialsId: 'postman-api-key', variable: 'POSTMAN_API_KEY'),
                        ...
                        withEnv(["environment_id=${TARGET_ENVIRONMENTS[params.environment]}"]) {
                                sh '''
                                    ...
                                    npx newman run https://api.getpostman.com/collections/${COLLECTION_ID}?apikey=${POSTMAN_API_KEY} --verbose --environment https://api.getpostman.com/environments/${environment_id}?apikey=${POSTMAN_API_KEY} --env-var jwt_pub_key="${PROCESSED_PUBLIC}" --env-var jwt_priv_key="${JWT_PRIVATE_KEY}" --delay-request ${delay} --insecure
                                '''
                            } 
                        //}
                    }
